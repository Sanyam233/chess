...
const Square = (props) => {
  ...
  const moves = () => {
    ...
    if (props.isPotential) {
      props.onBoardUpdate(props.pos);
    }
  };

  return (
    <div
      className={`square ${tileColor}`}
      onClick={moves}
    >
      <span className = {`square-text ${numberColor} top`} >{props.number}</span>
      {props.isPotential && <div className = {props.hasOtherPlayerPiece ? 'potential-contains-piece' : 'potential'}></div>}
